...
// src/db/queries.ts
async function updateAnimalCompound(animalId, compoundId) {
  const pool2 = getPool();
  const queryText = 'UPDATE "Tier" SET gehege_id = $1 WHERE id = $2 RETURNING *';
  const values = [compoundId, animalId];
  const result = await pool2.query(queryText, values);
...
var assignAnimalToCompound = async (c) => {
  try {
    const animalId = Number(c.req.param("id"));
...
// src/route/animal-route.ts
var animalsRouter = new Hono2;
